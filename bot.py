...
#         """Get symbol."""
#         return self.session.get("%s/public/symbol/%s" % (self.url, symbol_code), verify=False).json()

    def get_trades(self, symbol_code):
        """Get trades."""
        return self.session.get("%s/public/trades/%s" % (self.url, symbol_code), verify=False).json()

#     def get_orderbook(self, symbol_code):
#         """Get orderbook. """
#         return self.session.get("%s/public/orderbook/%s" % (self.url, symbol_code), verify=False).json()
#
#     def get_address(self, currency_code):
#         """Get address for deposit."""
#         return self.session.get("%s/account/crypto/address/%s" % (self.url, currency_code), verify=False).json()
#
#     def get_account_balance(self):
#         """Get main balance."""
#         return self.session.get("%s/account/balance" % self.url, verify=False).json()
#
#     def get_trading_balance(self):
#         """Get trading balance."""
#         return self.session.get("%s/trading/balance" % self.url, verify=False).json()
#
